...
return random_list = journalist.sample(5)

end

def bonjour_each(a)
a.each do |journalist| 
  # login_twitter.update "Bien le bonjour #{journalist} !  #bonjour_monde"
  puts "Bien le bonjour #{journalist} !  #bonjour_monde"
end
end
...
def follow(client)
  client.search('#bonjour_monde').take(5).each do |tweet|
...
login_twitter_streaming.filter(track: topics.join(",")) do |tweet|
  puts "tweet trouvé"
  puts tweet.text
